...
export const getGeminiConfig = async (token: string = '') => {
	...
	const res = await fetch(`${WEBUI_BASE_URL}/gemini/config`, {
		method: 'GET',
		headers: {
...
export const updateGeminiConfig = async (token: string = '', config: GeminiConfig) => {
	...
	const res = await fetch(`${WEBUI_BASE_URL}/gemini/config/update`, {
		method: 'POST',
		headers: {
			Accept: 'application/json',
			'Content-Type': 'application/json',
...
export const getGeminiModels = async (token: string, urlIdx?: number) => {
	...
	const res = await fetch(
		`${WEBUI_BASE_URL}/gemini/models${typeof urlIdx === 'number' ? `/${urlIdx}` : ''}`,
		{
			method: 'GET',
			headers: {
				Accept: 'application/json',
...
export const verifyGeminiConnection = async (
	...
	if (direct) {
		// Direct connection mode (for user settings)
		res = await fetch(`${url}/models?key=${key}`, {
			method: 'GET',
			headers: {
				Accept: 'application/json',
				'Content-Type': 'application/json'
		...
		// Admin verification via backend
		res = await fetch(`${WEBUI_BASE_URL}/gemini/verify`, {
			method: 'POST',
			headers: {
				Accept: 'application/json',
...
export const generateGeminiChatCompletion = async (
	token: string = '',
	body: object,
	url: string = `${WEBUI_BASE_URL}/gemini`
) => {
	let error = null;
